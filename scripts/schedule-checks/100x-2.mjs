#!/usr/bin/env node
// 战略档空跑门：先 best-effort 回收卡死的 processing 条目（不烧 attempt），再看 strategic lane
...
const LOG = join(homedir(), '.100x_v3', 'gate_strategic_lane.log');
const TIMEOUT_MS = 30_000;

function log(msg) {
  try { appendFileSync(LOG, `${new Date().toISOString()} [strategic-gate] ${msg}\n`); } catch {}
}
...
const r = spawnSync('python3', ['scripts/wechat_outbox.py', 'has-pending', '--lane', 'strategic'], {
  cwd: PROJECT,
  timeout: TIMEOUT_MS,
  stdio: ['ignore', 'pipe', 'pipe'],
});
if (r.status === 0) {
  log('strategic lane 有待推送条目，放行本轮');
  console.error('strategic outbox has pending items');
  process.exit(0);
}
if (r.status === 2) {
  log('strategic lane 无待推送条目，跳过本轮');
  console.error('strategic outbox empty');
  process.exit(2);
}
