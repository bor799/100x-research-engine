...
### 2.5 RSS 抓取问题（2026-04-29 发现）

**问题**：39 个 RSS 源中 17 个失败（44%），主要返回 HTTP 403。

**根本原因**：
- V3 的 `RSSAdapter` 使用 `urllib.request.urlopen()`，没有设置任何 HTTP 请求头
- 没有 User-Agent，导致很多网站拒绝访问

**验证结果**：
| 方式 | Gary Marcus | Jeff Geerling | John D Cook |
|------|-------------|--------------|-------------|
| 当前 RSSAdapter | ❌ 403 | ❌ 403 | ❌ 403 |
| + 浏览器 User-Agent | ✅ 20篇 | ✅ 20篇 | ✅ 30篇 |

**为什么不只用 User-Agent 补丁**：
- 局部优化，只解决 RSS，其他 fetcher 仍有问题
- 代码重复：每个地方都要设置 User-Agent、代理、超时
- 不一致：RSS、Web、Agent Reach 各用各的 HTTP 客户端

**系统性方案**（详见 `docs/RSS_FETCH_ARCHITECTURE_ANALYSIS.md`）：
1. 创建统一的 HttpClient 层（User-Agent、代理、重试）
2. 改造 RSSAdapter 使用 HttpClient
3. 整合 Agent Reach 作为首选抓取方式（支持 Jina Reader fallback）
4. 实现 Fetcher Router，根据 URL 自动选择最佳 Fetcher
5. V3 native Agent Reach 实现（移除 sys.path 依赖）

**预期效果**：RSS 成功率从 56% 提升到 95%+
...
### P0：统一 HTTP 客户端与 RSS 抓取修复（新增，高优先级）

1. 创建统一 HttpClient
   - 新建 `src/knowledge_extractor_v3/fetchers/http_client.py`
   - 支持：User-Agent、代理、超时、重试、Jina Reader fallback
   - 统一错误处理和 TypedError 转换
   - 复用 WebPageFetcher 的 User-Agent 配置

2. 改造 RSSAdapter
   - RSSAdapter 改用 HttpClient（而非直接 urllib）
   - 失败时自动降级到 Jina Reader
   - 验证 RSS 成率达到 95%+

3. 实现 Fetcher Router
   - 新建 `src/knowledge_extractor_v3/fetchers/router.py`
   - 根据 URL/domain 自动选择：Agent Reach > Web > Fixture
   - Worker 默认使用 Router，而非单一 WebPageFetcher

4. V3 Native Agent Reach
   - 将 V2 的 ar_channels 迁移到 V3
   - 新目录：`src/knowledge_extractor_v3/channels/`
   - 移除 `multi_channel.py` 的 sys.path 依赖
   - 所有 Channel 返回 V3 的 FetchedContent 格式
...
3. RSS adapter（新增系统性要求）
   - **使用统一 HttpClient**（而非直接 urllib）
   - **支持 User-Agent**，解决 44% 的 403 失败问题
   - **Jina Reader fallback**，当直接请求失败时自动降级
...
### Phase 2：统一 HTTP 客户端与 RSS 抓取修复

完成标准：

- 创建 `src/knowledge_extractor_v3/fetchers/http_client.py`
- HttpClient 支持：User-Agent、代理、重试、Jina Reader fallback
- RSSAdapter 改用 HttpClient，移除直接 urllib 调用
- RSS 抓取成功率 > 95%（验证 39 个源）
- 实现 Fetcher Router，根据 URL 自动选择 Agent Reach / Web / Fixture
- Worker 默认使用 Fetcher Router

### Phase 2.5：把 V2 RSS 全量接入 V3
...
### Phase 3：V3 Native Agent Reach

完成标准：

- 创建 `src/knowledge_extractor_v3/channels/` 目录
- 迁移 V2 的 ar_channels 到 V3（YouTube、Twitter、WeChat、Xiaoyuzhou、Web）
- 移除 `multi_channel.py` 的 sys.path 依赖
- 所有 Channel 返回 V3 的 FetchedContent 格式
- Agent Reach 健康检查集成到 V3 health 命令
...
- [ ] `python -m pytest` 全绿。
- [ ] **RSS 抓取成功率 > 95%**（当前 56%）。
- [ ] **统一 HttpClient 已实现并使用**。
- [ ] **Fetcher Router 已集成到 Worker**。
- [ ] **V3 Native Agent Reach 已实现**（移除 sys.path 依赖）。
...
建议下一 session 优先处理 RSS 抓取问题（高优先级），然后再继续其他迁移任务：

### 优先级 1：RSS 抓取修复

1. **创建统一 HttpClient**
   - 新建 `src/knowledge_extractor_v3/fetchers/http_client.py`
   - 支持 User-Agent、代理、重试、Jina Reader fallback
   - 参考 WebPageFetcher 的 User-Agent 配置

2. **改造 RSSAdapter**
   - 修改 `src/knowledge_extractor_v3/sources/rss.py`
   - 使用 HttpClient 替代直接 urllib 调用
   - 失败时自动降级到 Jina Reader
...
### 优先级 2：Fetcher Router

4. **实现 Fetcher Router**
   - 新建 `src/knowledge_extractor_v3/fetchers/router.py`
   - 根据 URL 自动选择：Agent Reach > Web > Fixture

5. **集成到 Worker**
   - Worker 默认使用 Fetcher Router
   - 移除硬编码的 WebPageFetcher

### 优先级 3：原有迁移任务（按顺序）

6. 修复 `tests/test_worker.py` 的 `signal` import。
7. 修复 `src/knowledge_extractor_v3/scheduler.py` 的 log path suffix。
8. 修复 `scripts/run-v3.sh` 权限与 `scripts/run.sh` 的错误命令。
9. 修复 `scripts/nightly_job.sh`，改为 scheduler + worker。
10. 增加 source config 外部文件加载，把 `config/sources.yaml` 的 91 个 RSS 源纳入正式配置。

### 优先级 4：V3 Native Agent Reach

11. 创建 `src/knowledge_extractor_v3/channels/` 目录
12. 迁移 V2 的 ar_channels 到 V3
13. 移除 `multi_channel.py` 的 sys.path 依赖
...
**相关文档**：
- `docs/RSS_FETCH_ARCHITECTURE_ANALYSIS.md` - RSS 抓取架构详细分析
