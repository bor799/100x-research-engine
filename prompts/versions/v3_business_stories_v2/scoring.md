# V3 Business Stories Scoring Prompt (v2)
...
对输入内容做质量筛选，并补充信源评分与内容压缩字段：

1. 判断内容是否有真实增量，而不是普通新闻、营销稿或二手转述。
2. 评估信源的可验证链条、利益关系和可验证性。
...
| 宏观/加密资产/AI 基建 | 1.0 | 深入剖析宏观走势、AI 算力/存储/应用、数据飞轮标的、加密市场逻辑与投资机会 |
| 内幕/运作机制 | 1.5 | 揭露行业水下机制、非公开利益链条、真实成本结构或运作逻辑 |
| 产品思维/增量洞察 | 1.5 | 具有深刻用户视角、产品判断或反常识但可验证的洞察 |
| 个人成长/重要人物一手访谈 | 1.5 | 有拿到结果的人分享真金白银经验，尤其是经营、心理、长期主义和决策经验 |
...
**关键原则：不熟悉不等于不可信。** 判断 StrongSecondary/Primary 的依据是机制
（署名作者、编辑判断、一手访谈、可核对的事实链），不是该媒体在英文世界是否有名，
也不是你是否认识它。中文专业媒体的严肃行业报道、机构署名访谈、带数据来源的深度
分析，与英文同类内容同等对待。

- 具名机构投资人/创始人接受专业媒体署名访谈：`IndustryInterview`，链路完整时
  `StrongSecondary`，`L1` 不低于 0.55。
- 专业中文科技/创投媒体（如 elsewhere.news 及同类）的编辑性深度报道：
  `IndustryMedia`，`StrongSecondary`，`L1` 不低于 0.55。
- `Unknown` 只用于真正无法验证的链条（匿名论坛帖、无署名转载），不用于
  "没听过但专业" 的信源。

`interest_flag=PositionTalking` 表示内容带机构立场，通过 D4/L1 适度折价体现，
**不允许** 因此把 L1 压到 0.2 以下或触发一票否决；利益冲突是否压过证据可信度，
取决于内容是否只剩宣传而没有可核对的事实。
...
- `StrongSecondary`: 专业媒体、专家研究、严肃访谈、带编辑判断的行业报道（任何语言）。
...
下面五个字段各是 0-1 的数字，必须独立打分。它们由代码按固定权重合成
`business_story_fit`，是商业主推的唯一可计算依据——不要在散文里写"权重 2.0"之类
模型无法执行的描述，只给这五个数值。

- `actor_scene`: 是否有具体的人/公司/场景。空泛行业叙述 0；具名主体和场景 1。
- `operating_detail`: 经营机制有多具体。定价、获客、成本、利润、团队、增长动作等一线细节越多分越高。融资额度本身不算经营机制。
- `causal_arc`: 是否有可复述的起因-冲突-结果链条。只罗列事件 0；有清晰因果和转折 1。
- `transferability`: 经验能否被他人复用或转述。一次性新闻 0；可迁移方法或教训 1。
- `evidence_strength`: 证据强度。营销稿/单一匿名 0；可验证数据/一手披露 1。

注意：融资 PR 即使有人名、公司和数字，只要缺少经营机制（`operating_detail` 低）或因果过程
（`causal_arc` 低），就不会被推送到微信——它可能仍因 `final_score` 进入战略摘要或归档。
...
- `L2`: 事实/主张的出处强度，0-1。署名访谈、可核对数据、一手案例给高分。
- `L3`: 内容质量、信息增量和稀缺性，0-1。
- `L4`: 与用户关注领域和行动价值的匹配度，0-1。

`L1`、`L2`、`L3`、`L4` 各自独立、如实打分，不要为了控制总分而互相压低。

系统会在代码里按以下公式合成（你也要输出一致的数值，但代码以自己的计算为准）：

- `objective_quality = 0.40*L1 + 0.30*L2 + 0.30*L3`。
...
- `A`: `final_score >= 0.70`，值得进入深度萃取。
- `B`: `0.50 <= final_score < 0.70`，边缘，不默认进入萃取，需要人工或后续证据。
- `C`: `0.30 <= final_score < 0.50`，低优先级观察。
...
- 利益冲突强到内容只剩宣传、没有任何可核对事实。

注意：不要再因为"宏大叙事"就否决内容。高价值的宏观/战略分析即使没有具体小商业故事，
也可能因 `final_score` 进入战略摘要（`strategic_digest`）或归档（`archive_only`）。
是否推送到微信由代码按商业故事维度判定，不由这里的一票否决决定。
